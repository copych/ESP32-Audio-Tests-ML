...
void adcRead( int show_serial , uint16_t &param_val0 , uint16_t &param_val1 , uint16_t &param_val2 , uint16_t &param_val3  ){
  // store old value in a variable
  adc0 = analogRead(POT0);
  adc1 = analogRead(POT1);
  adc2 = analogRead(POT2);
  adc3 = analogRead(POT3);
  adc0_1 = adc0;
  adc1_1 = adc1;
  adc2_1 = adc2;
  adc3_1 = adc3;
  // fetch new values with a slightly lowpass and better precision
  adc0 = ( adc0 + analogRead(POT0) ) / 2;
  // delay(1);
  adc1 = ( adc1 + analogRead(POT1)  ) / 2;
  // delay(1);
  adc2 = ( adc2 + analogRead(POT2)  ) / 2;
  // delay(1);
  adc3 = ( adc3 + analogRead(POT3)  ) / 2;


  if( adc0 > adc0_1 +adc_slope || adc0<adc0_1 -adc_slope ){
    param_val0 = map( adc0, 0, ADC_MAX, 0, 127 );
  ...
  if( adc1 > adc1_1 +adc_slope || adc1<adc1_1 - adc_slope ){
    param_val1 = map( adc1, 0, ADC_MAX, 0, 127 );
    if( val1_synced == 1 ){
      switch( act_menuNum ){
        case 0:
          if( act_instr > 0  ){
        ...
        case 1:
          if(  act_instr > 0 ){
            param_val1 = map( adc1, 0, ADC_MAX, 1, 16 ); // Change MidiChannel Out
  ...
  if( adc2 > adc2_1 +adc_slope || adc2<adc2_1 -adc_slope ){
    param_val2 = map( adc2, 0, ADC_MAX, 0, 127 );
    if( val2_synced == 1 ){
      switch( act_menuNum ){
        case 0:
          if( act_instr > 0 ){
  ...
  if( adc3 > adc3_1 +adc_slope || adc3<adc3_1 -adc_slope ){ 
    param_val3 = map( adc3, 0, ADC_MAX, 0, 127 );
    if( val3_synced == 1 ){
      switch( act_menuNum ){
        case 0:
          if( act_instr > 0 ){
